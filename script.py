...
def chooseSentencesFromCorpus(files):
  ...
  for pathStr in files:
    ...
    for text_str in corpus:
      ...
      text = Text(text_str)
      text.tag_analysis()
      for word in text.words:       #Kui sõnal on rohkem kui 1 analüüs siis lausei ei sobi
        if len(word[ANALYSIS]) > 1:
          disambugation = False
      if disambugation == True:
        sentence_analysis = text.get.word_texts.lemmas.forms.root_tokens.named_entities.as_dict
        root_tokens =text.get.root_tokens.as_dict
        #clitic = text.get.clitics.as_dict
        #print(clitic)
...
def runCaseAnalys(case_dict, list_of_sentences,inappropriateWords):
  ...
  if len(list_of_sentences)>0:
    for sentence_info in list_of_sentences:
      ...
      for root in roots:                                  #kontroll kas liitsõnas on sõna, mis on ebasobilike sõnade hulgas
          if len(root)> 1:
            for r in root:
              text = Text(r)
              text.tag_analysis()
              lemma=(text.lemmas)[0]
              if lemma in inappropriateWords:
          ...
          if form != '':
            ...
            if nominative not in inappropriateWords:
              ...
              if case_info[0]=='adt':                           #Lühikesisseütlev
                ...
                go = True
              if go == True and name == True:
                sen_x = re.sub(' ' + word + ' ',' %%% ',sentence)
                if casename != "n" or casename =="n" and sg_pl == "pl":
                  (content_all) = addToContent(word, content_all, casename, id, nominative, sen_x,sg_pl,title)
                  added = True
                if casename == "n" and sg_pl == "pl":
                  content_n = addToContent(word, content_n, casename, id, nominative, sen_x, sg_pl,title)
                  added = True
                if casename == "g" or casename=="es":
                  (content_g_es)= addToContent(word, content_g_es, casename, id, nominative, sen_x,sg_pl,title)
                  added = True
                if casename == "p":
                  (content_p)= addToContent(word, content_p, casename, id, nominative, sen_x,sg_pl,title)
                  added = True
                if casename == "ill" or casename == "in" or casename == "el" or casename == "adt" or casename == "all" or casename == "ad" or casename == "abl":
                  (content_ill)= addToContent(word, content_ill, casename, id, nominative, sen_x,sg_pl,title)
                  added = True
                if casename == "tr" or casename=="ter" or casename=="ab" or casename=="kom":
                  (content_tr_ter_ab_kom) = addToContent(word, content_tr_ter_ab_kom, casename, id, nominative, sen_x,sg_pl,title)
                  added = True                
                go = False
            else:
              appropriateSentence = False
...
def addToContent(word, content, casename, countid, nominative, sen_x,sg_pl,title):
