...
allowedPOS = ['S','V','A','C','U','J','K','N','O','Z'] #nimi-, tegu-, omadus-, .... lubatud
...
def chooseSentencesFromCorpus(files,freqWordList,inappropriateWords,allowedPOS):
  combinations3words = {}
  combinations4words = {}
  combinations5words = {}
  combinations6words = {}
    ...
    for text in texts:
      ...
      if sentenceLen > 2 and sentenceLen < 8 and 'V' in posList :
        if posList[0] == 'J' or posList[0]== 'V' or wordsList[0][0].isupper()==False: # Kui lause algab sidesõna või tegusnaga sisi eisobi ning kui lause ei alga suure tähega
          ...
          for index in range(len(text.words)):       #Kui sõnal on rohkem kui 1 analüüs, siis lausei ei sobi, sest lause pole üheselt määratud    
...
def main():
  ...
  (combin3,combin4,combin5,combin6,combin7,sentences_with_structure_3, sentences_with_structure_4, sentences_with_structure_5, sentences_with_structure_6,sentences_with_structure_7) = chooseSentencesFromCorpus(files,freqWordList,inappropriateWords,allowedPOS)
